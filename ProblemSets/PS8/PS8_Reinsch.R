...
summary(estimate)
#Btrue=0.7748
betols <- solve(crossprod(X))%*%crossprod(X,Y)
betols
#This beta is very close, almost identical to the real beta
...
print(result)
# The results between the LBFGS and the Nelder-Mead tests did not differ. 
...
# Regression
estimate<-lm(Y~X-1)
modelsummary(estimate, output="ps8.tex")
